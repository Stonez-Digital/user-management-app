...
export default function EnrollmentPage(){
  ...
  const enrollmentSessions=sessions.filter(s=>s.status==="planned"||s.status==="active");
  ...
  return <div className="content standalone">
    ...
    <section className="panel"><div className="panel-head"><div><h2>Enrollment records</h2><p>{filtered.length} enrollment record{filtered.length===1?"":"s"} · {items.filter(e=>e.status==="active").length} active</p></div><input className="search-inline" placeholder="Search..." value={query} onChange={e=>setQuery(e.target.value)}/></div>
