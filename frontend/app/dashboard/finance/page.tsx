...
function FinanceContent(){
 ...
 <section className="grid-2">
  ...
  <Panel title="Create invoice" text="Invoice an active student enrollment using a selected term."><form className="form-grid" onSubmit={e=>{e.preventDefault();if(!invoice.student_enrollment_id||!invoice.term_id||!invoice.due_date||!invoice.description||Number(invoice.quantity)<=0||Number(invoice.unit_amount)<=0){setError("Complete all invoice fields before creating the invoice.");return}setError("");submit("/admin/invoices",{student_enrollment_id:invoice.student_enrollment_id,term_id:invoice.term_id,due_date:invoice.due_date,lines:[{description:invoice.description.trim(),quantity:Number(invoice.quantity),unit_amount:Number(invoice.unit_amount)}]},()=>setInvoice({...invoice,student_enrollment_id:"",due_date:"",description:"",quantity:"1",unit_amount:""}))}}>
   ...
   <label>Term<select required value={invoice.term_id}onChange={e=>{const id=e.target.value;const t=terms.find(x=>x.id===id);setInvoice({...invoice,term_id:id,due_date:t?.end_date?t.end_date.slice(0,10):invoice.due_date})}}><option value="">Select term</option>{terms.map(t=><option key={t.id}value={t.id}>{t.name}</option>)}</select></label<label>Fee item<select value={invoice.description&&invoice.unit_amount?`${invoice.description}|${invoice.unit_amount}`:""} onChange={e=>{const [description,unit_amount]=e.target.value.split("|");setInvoice({...invoice,description:description||"",unit_amount:unit_amount||""})}}><option value="">Select fee item or enter manually</option>{fees.filter(x=>!invoice.term_id||x.term_id===invoice.term_id).map(x=><option key={x.id} value={`${x.name}|${x.amount}`}>{x.name} — {x.amount}</option>)}</select></label>
   >
   ...
   <div className="form-action"><button type="submit" disabled={busy}>{busy?"Creating invoice…":"Create invoice"}</button></div>
  </form></Panel>
 </section>
