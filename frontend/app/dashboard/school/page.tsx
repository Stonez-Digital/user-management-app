...
export default function SchoolSettings(){
 ...
 useEffect(()=>{api("/me").then(me=>{if(me?.role!=="school_admin"){router.replace(me?.role==="super_admin"?"/platform/schools":"/dashboard");return} return api("/admin/school")}).then(s=>{if(s){setSchool(s);setName(s.name);setLogoURL(s.logo_url||"");setStatus(s.status)}}).catch(e=>{setError(e.message);if(e.message==="Session expired")router.replace("/")})},[router]);

 async function save(e:React.FormEvent){e.preventDefault();setSaving(true);setError("");try{const s=await api("/admin/school",{method:"PUT",body:JSON.stringify({name,logo_url:logoURL})});setSchool(s);setName(s.name);setStatus(s.status)}catch(e){setError(e instanceof Error?e.message:"Unable to save school")}finally{setSaving(false)}}
 ...
 <form onSubmit={save} className="form-grid school-form"><label>School name<input value={name} onChange={e=>setName(e.target.value)} placeholder="e.g. Stonez Digital Academy" minLength={2} maxLength={160} required/></label><label>School logo URL<input type="url" value={logoURL} onChange={e=>setLogoURL(e.target.value)} placeholder="https://.../school-logo.png" maxLength={1000}/><small className="muted">Use a persistent HTTPS image URL. A Stonez Digital fallback is used when none is configured.</small></label><div className="form-action"><button disabled={saving}>{saving?"Saving…":"Save school profile"}</button></div></form><div className="tenant-note"><strong>School login</strong><span>/school/{school?.slug||"your-school"}/login</span></div></section>
