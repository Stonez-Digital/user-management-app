...
export default function TeacherDashboard(){
  ...
  const [timetable,setTimetable]=useState<Timetable[]>([]); const [sessions,setSessions]=useState<Session[]>([]); const [terms,setTerms]=useState<Term[]>([]);
  const [error,setError]=useState("");
  useEffect(()=>{(async()=>{try{
    const m=await api("/me"); if(m?.role!=="teacher"){router.replace("/dashboard");return} setMe(m);
    const [a,t,s]=await Promise.all([api("/teacher/assignments"),api("/teacher/timetable"),api("/admin/academic-sessions")]);
    setAssignments(list(a,"assignments")); setTimetable(list(t,"timetable")); const ss=list(s,"sessions"); setSessions(ss);
    const active=ss.find((x:Session)=>x.status==="active")||ss[0]; if(active){const td=await api("/admin/academic-sessions/"+active.id+"/terms");setTerms(list(td,"terms"))}
