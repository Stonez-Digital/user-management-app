...
export default function LoginPage() {
 ...
 async function login(e:React.FormEvent){
  ...
  try {
   ...
   if(!me.ok) throw Error(user?.error?.message||user?.error||"Unable to load account");
 ...
 return <main className="auth-shell"><section className="auth-card"><div className="brand-mark">S</div><p className="eyebrow">STONEZ DIGITAL</p><h1>School Management</h1><p className="muted">Sign in to manage your school operations.</p><form onSubmit={login} className="form"><label>Email<input type="email" value={email} onChange={e=>setEmail(e.target.value)} placeholder="admin@school.com" required/></label><label>Password<input type="password" value={password} onChange={e=>setPassword(e.target.value)} placeholder="••••••••" required/>{error&&<div className="error">{error}</div>}<button disabled={loading}>{loading?"Signing in...":"Sign in"}</button></form></section></main>;
}
