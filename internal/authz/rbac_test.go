...
func TestSchoolRolesAreValid(t *testing.T) {
    roles := []string{
        RoleSuperAdmin, RoleSchoolAdmin, RoleTeacher, RoleStudent,
        RoleParent, RoleAccountant, RoleStaff,
    }
...
func TestSuperAdminHasAllDefinedPermissions(t *testing.T) {
    ...
    for _, permission := range permissions {
        if !HasPermission(RoleSuperAdmin, permission) {
            t.Fatalf("super admin should have %q", permission)
        }
    }
}

func TestSchoolAdminCannotAssignRolesOrDeleteUsers(t *testing.T) {
    if HasPermission(RoleSchoolAdmin, PermissionRolesAssign) {
        t.Fatal("school admin must not assign roles")
    }
    if HasPermission(RoleSchoolAdmin, PermissionUsersDelete) {
        t.Fatal("school admin must not delete users")
    }
    if !HasPermission(RoleSchoolAdmin, PermissionUsersRead) {
        t.Fatal("school admin should read users")
    }
}
    ...
    for _, role := range roles {
        for _, permission := range permissions {
            if HasPermission(role, permission) {
                t.Fatalf("role %q unexpectedly has %q", role, permission)
            }
        }
    }
}
