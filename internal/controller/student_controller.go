...
func (ctrl *StudentController) Create(c *gin.Context) {
    ...
    student, err := ctrl.service.CreateStudent(models.Student{UserID:req.UserID, AdmissionNumber:req.AdmissionNumber, DateOfBirth:req.DateOfBirth, Gender:req.Gender, GuardianName:req.GuardianName, GuardianPhone:req.GuardianPhone, GuardianEmail:req.GuardianEmail, EnrollmentStatus:req.EnrollmentStatus})
    if err != nil {
        switch {
...
func (ctrl *StudentController) List(c *gin.Context) { students,err:=ctrl.service.GetStudents(); if err!=nil { httpx.Error(c,500,"student_list_failed","failed to load students"); return }; c.JSON(http.StatusOK,students) }
func (ctrl *StudentController) Get(c *gin.Context) {
    ...
    student,err:=ctrl.service.GetStudent(id); if errors.Is(err,service.ErrStudentNotFound) { httpx.Error(c,404,"student_not_found","student not found"); return }; if err!=nil { httpx.Error(c,500,"student_get_failed","failed to load student"); return }; c.JSON(http.StatusOK,student)
}
func (ctrl *StudentController) Update(c *gin.Context) {
    ...
    student,err:=ctrl.service.GetStudent(id); if errors.Is(err,service.ErrStudentNotFound) { httpx.Error(c,404,"student_not_found","student not found"); return }; if err!=nil { httpx.Error(c,500,"student_get_failed","failed to load student"); return }
    ...
    if err:=ctrl.service.UpdateStudent(student); err!=nil { if errors.Is(err,service.ErrDuplicateAdmission) { httpx.Error(c,409,"duplicate_admission_number","admission number already exists"); return }; if errors.Is(err,service.ErrStudentNotFound) { httpx.Error(c,404,"student_not_found","student not found"); return }; httpx.Error(c,500,"student_update_failed","failed to update student"); return }
...
func (ctrl *StudentController) Delete(c *gin.Context) {
    id,err:=uuid.Parse(c.Param("id")); if err!=nil { httpx.Error(c,400,"invalid_student_id","invalid student id"); return }
    if err:=ctrl.service.DeleteStudent(id); err!=nil { if errors.Is(err,service.ErrStudentNotFound) { httpx.Error(c,404,"student_not_found","student not found"); return }; httpx.Error(c,500,"student_delete_failed","failed to delete student"); return }
