...
func(ctrl *SubjectController)List(c *gin.Context){schoolID,ok:=requireSchoolID(c);if !ok{return};v,e:=ctrl.service.List(schoolID);if e!=nil{subjectError(c,e);return};c.JSON(200,v)}
func(ctrl *SubjectController)Get(c *gin.Context){schoolID,ok:=requireSchoolID(c);if !ok{return};id,e:=uuid.Parse(c.Param("id"));if e!=nil{httpx.Error(c,400,"invalid_subject_id","invalid subject id");return};v,e:=ctrl.service.Get(schoolID,id);if e!=nil{subjectError(c,e);return};c.JSON(200,v)}
func(ctrl *SubjectController)Create(c *gin.Context){schoolID,ok:=requireSchoolID(c);if !ok{return};var r subjectRequest;if e:=c.ShouldBindJSON(&r);e!=nil{httpx.Validation(c,httpx.ValidationErrors(e));return};active:=true;if r.Active!=nil{active=*r.Active};v,e:=ctrl.service.Create(schoolID,models.Subject{Code:r.Code,Name:r.Name,Description:r.Description,Active:active});if e!=nil{subjectError(c,e);return};actor,_:=uuid.Parse(c.GetString("user_id"));_=audit.Record(ctrl.service.DB(),c,&actor,"subject.create","subject",&v.ID,nil);c.JSON(http.StatusCreated,v)}
func(ctrl *SubjectController)Update(c *gin.Context){schoolID,ok:=requireSchoolID(c);if !ok{return};id,e:=uuid.Parse(c.Param("id"));if e!=nil{httpx.Error(c,400,"invalid_subject_id","invalid subject id");return};var r subjectRequest;if e=c.ShouldBindJSON(&r);e!=nil{httpx.Validation(c,httpx.ValidationErrors(e));return};v,e:=ctrl.service.Get(schoolID,id);if e!=nil{subjectError(c,e);return};v.Code,v.Name,v.Description=r.Code,r.Name,r.Description;if r.Active!=nil{v.Active=*r.Active};if e=ctrl.service.Update(schoolID,v);e!=nil{subjectError(c,e);return};actor,_:=uuid.Parse(c.GetString("user_id"));_=audit.Record(ctrl.service.DB(),c,&actor,"subject.update","subject",&v.ID,nil);c.JSON(200,v)}
func(ctrl *SubjectController)Delete(c *gin.Context){schoolID,ok:=requireSchoolID(c);if !ok{return};id,e:=uuid.Parse(c.Param("id"));if e!=nil{httpx.Error(c,400,"invalid_subject_id","invalid subject id");return};if e=ctrl.service.Delete(schoolID,id);e!=nil{subjectError(c,e);return};actor,_:=uuid.Parse(c.GetString("user_id"));_=audit.Record(ctrl.service.DB(),c,&actor,"subject.delete","subject",&id,nil);c.JSON(200,gin.H{"message":"subject deleted"})}