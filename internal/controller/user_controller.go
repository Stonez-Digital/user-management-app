...
func (ctrl *UserController) GetMe(c *gin.Context) {
	school,ok:=schoolID(c);if !ok{return};id,err:=uuid.Parse(c.GetString("user_id"));if err!=nil{httpx.Error(c,400,"invalid_user_id","invalid user id");return}
	user,err:=ctrl.service.GetUser(school,id);if err!=nil{httpx.Error(c,404,"user_not_found","user not found");return};c.JSON(200,user)
}
