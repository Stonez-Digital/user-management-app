...
const ( AcademicStatusPlanned="planned"; AcademicStatusActive="active"; AcademicStatusClosed="closed"; AcademicStatusArchived="archived" )
const ( TermFirst="first"; TermSecond="second"; TermThird="third" )
...
type Term struct {
 ID uuid.UUID `gorm:"type:uuid;primaryKey" `json:"id"`
 AcademicSessionID uuid.UUID `gorm:"type:uuid;not null;index;uniqueIndex:uq_session_term" `json:"academic_session_id"`
 AcademicSession AcademicSession `gorm:"foreignKey:AcademicSessionID;constraint:OnUpdate:CASCADE,OnDelete:CASCADE" `json:"-"`
 Name string `gorm:"size:20;not null;uniqueIndex:uq_session_term" `json:"name"`
 StartDate time.Time `gorm:"not null" `json:"start_date"`
 EndDate time.Time `gorm:"not null" `json:"end_date"`
 Status string `gorm:"size:20;not null;default:planned;index" `json:"status"`
 CreatedAt time.Time `json:"created_at"`
