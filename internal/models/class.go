...
type SchoolClass struct {
 ...
 SchoolID uuid.UUID `gorm:"type:uuid;not null;index;uniqueIndex:uq_school_class_name" json:"school_id"`
 School School `gorm:"foreignKey:SchoolID;constraint:OnUpdate:CASCADE,OnDelete:RESTRICT" json:"-"`
 Name string `gorm:"size:100;not null;uniqueIndex:uq_school_class_name" json:"name"`
...
type Section struct {
 ...
 School School `gorm:"foreignKey:SchoolID;constraint:OnUpdate:CASCADE,OnDelete:RESTRICT" json:"-"`
 ClassID uuid.UUID `gorm:"type:uuid;not null;index;uniqueIndex:uq_school_class_section" json:"class_id"`
 Name string `gorm:"size:50;not null;uniqueIndex:uq_school_class_section" json:"name"`
 CreatedAt time.Time `json:"created_at"`
 UpdatedAt time.Time `json:"updated_at"`
}
