...
type EnrollmentRepository interface {
	...
	History(uuid.UUID, uuid.UUID) ([]models.StudentEnrollment, error)
...
func (r *enrollmentRepo) History(schoolID,studentID uuid.UUID) ([]models.StudentEnrollment,error) { var out []models.StudentEnrollment; err:=r.db.Where("school_id = ? AND student_id = ?",schoolID,studentID).Preload("Student.User","school_id = ?",schoolID).Preload("AcademicSession","school_id = ?",schoolID).Preload("SchoolClass","school_id = ?",schoolID).Preload("Section","school_id = ?",schoolID).Order("enrolled_at DESC").Find(&out).Error; return out,err }
