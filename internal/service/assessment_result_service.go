...
func(s *AssessmentResultService) TeacherCreate(schoolID,teacherID uuid.UUID,v models.AssessmentResult)(models.AssessmentResult,error){
 ...
 if e:=s.db.Preload("TeacherAssignment").Where("assessments.id = ? AND assessments.school_id = ?",v.AssessmentID,schoolID).First(&a).Error;e!=nil{return v,ErrResultAssessmentMissing}
...
func (s *AssessmentResultService) ReportCard(enrollmentID, termID uuid.UUID) (ReportCard, error) {
	...
	if e := s.db.Select("id, school_id").Where("id = ?", enrollmentID).First(&enrollment).Error; e != nil {
...
func (s *AssessmentResultService) ReportCardForSchool(schoolID, enrollmentID, termID uuid.UUID) (ReportCard, error) {
	...
	if e := s.db.Where("school_id = ? AND student_enrollment_id = ?", schoolID, enrollmentID).
		Preload("Assessment", "school_id = ?", schoolID).
		Preload("Assessment.TeacherAssignment", "school_id = ?", schoolID).
		Preload("Assessment.TeacherAssignment.Subject", "school_id = ?", schoolID).
	...
	for _, result := range results {
		...
		contribution := (result.Score / result.Assessment.MaxScore) * result.Assessment.Weight
		g.WeightedContribution += contribution
		totalScore += result.Score
		totalMax += result.Assessment.MaxScore
		weighted += contribution
		totalWeight += result.Assessment.Weight
	}
