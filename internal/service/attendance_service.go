...
func(s *AttendanceService) ListForTeacher(schoolID, teacherID, sessionID, termID uuid.UUID) ([]models.AttendanceRecord,error) {
...
func(s *AttendanceService)List(schoolID,sessionID,termID uuid.UUID)([]models.AttendanceRecord,error){rows,e:=s.repo.List(schoolID);if e!=nil{return nil,e};out:=make([]models.AttendanceRecord,0,len(rows));for _,r:=range rows{if r.TermID==termID&&r.Term.AcademicSessionID==sessionID{out=append(out,r)}};return out,nil}
