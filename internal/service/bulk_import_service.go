...
func(s *BulkImportService)processTeacher(j models.BulkImportJob,name,email string,row BulkRow)(bool,string,string,string,error){var created bool;var password string;userEmail:=email;var role=authz.RoleTeacher;e:=s.db.Transaction(func(tx *gorm.DB)error{staff:=strings.TrimSpace(row["staff_id"]);var p models.TeacherProfile;if e:=tx.Where("school_id=? AND staff_id=?",j.SchoolID,staff).First(&p).Error;e==nil{return nil}else if !errors.Is(e,gorm.ErrRecordNotFound){return e};u,pw,wasCreated,e:=s.createUser(tx,j.SchoolID,name,email,authz.RoleTeacher);password=pw;created=wasCreated;userEmail=u.Email;if e!=nil{return e};p=models.TeacherProfile{SchoolID:j.SchoolID,UserID:u.ID,StaffID:staff,Phone:row["phone"],Gender:row["gender"],Department:row["department"],Designation:row["designation"],Subjects:row["subjects"],Classes:row["classes"],Status:row["status"]};if e=tx.Create(&p).Error;e!=nil{return e};return nil});return created,userEmail,password,role,e}
