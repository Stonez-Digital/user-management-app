...
func(s *BulkImportService)run(j models.BulkImportJob){s.mu.Lock();defer s.mu.Unlock();var rows []BulkRow;if json.Unmarshal([]byte(j.RowsJSON),&rows)!=nil{s.fail(j.ID,"invalid stored import payload");return};j.Status=models.BulkImportRunning;s.db.Model(&j).Updates(map[string]any{"status":j.Status})
 ...
 for i,row:=range rows{rn:=i+2;if validSet[rn]{continue};created,email,password,role,e:=s.processRow(j,row);if e!=nil{errs=append(errs,BulkError{rn,"",e.Error()});j.Failed++}else{j.Created++;if j.SendCredentials&&created{if e=s.mailer.SendCredentials(strings.TrimSpace(row["name"]),email,role,password);e!=nil{errs=append(errs,BulkError{rn,"email","account created but credential email failed: "+e.Error()});j.CredentialEmailsFailed++}else{j.CredentialsSent++}}};s.db.Model(&j).Updates(map[string]any{"created":j.Created,"failed":j.Failed,"credentials_sent":j.CredentialsSent,"credential_emails_failed":j.CredentialEmailsFailed,"errors_json":mustJSON(errs)})}
 j.ErrorsJSON=mustJSON(errs);if j.Failed>0||len(errs)>0{j.Status=models.BulkImportCompletedWithErrors}else{j.Status=models.BulkImportCompleted};s.db.Model(&j).Updates(map[string]any{"status":j.Status,"errors_json":j.ErrorsJSON,"failed":j.Failed,"credentials_sent":j.CredentialsSent,"credential_emails_failed":j.CredentialEmailsFailed})}
...
func(s *BulkImportService)processRow(j models.BulkImportJob,row BulkRow)(bool,string,string,string,error){
...
func(s *BulkImportService)processStudent(j models.BulkImportJob,name,email string,row BulkRow)(bool,string,string,string,error){var created bool;var password,userEmail=email;var role=authz.RoleStudent;e:=s.db.Transaction(func(tx *gorm.DB)error{var st models.Student;ad:=strings.TrimSpace(row["admission_number"]);if e:=tx.Where("school_id=? AND admission_number=?",j.SchoolID,ad).First(&st).Error;e==nil{j.Skipped++;return nil}else if !errors.Is(e,gorm.ErrRecordNotFound){return e};u,p,wasCreated,e:=s.createUser(tx,j.SchoolID,name,email,authz.RoleStudent);password=p;created=wasCreated;userEmail=u.Email;if e!=nil{return e};st=models.Student{SchoolID:j.SchoolID,UserID:u.ID,AdmissionNumber:ad,EnrollmentStatus:models.EnrollmentActive,GuardianName:row["guardian_name"],GuardianPhone:row["guardian_phone"],GuardianEmail:row["guardian_email"]};if v:=row["gender"];v!=""{st.Gender=v};if e=tx.Create(&st).Error;e!=nil{return e};return s.maybeEnroll(tx,j.SchoolID,st.ID,row)});return created,userEmail,password,role,e}
...
func(s *BulkImportService)processTeacher(j models.BulkImportJob,name,email string,row BulkRow)(bool,string,string,string,error){var created bool;var password,userEmail=email;var role=authz.RoleTeacher;e:=s.db.Transaction(func(tx *gorm.DB)error{staff:=strings.TrimSpace(row["staff_id"]);var p models.TeacherProfile;if e:=tx.Where("school_id=? AND staff_id=?",j.SchoolID,staff).First(&p).Error;e==nil{return nil}else if !errors.Is(e,gorm.ErrRecordNotFound){return e};u,pw,wasCreated,e:=s.createUser(tx,j.SchoolID,name,email,authz.RoleTeacher);password=pw;created=wasCreated;userEmail=u.Email;if e!=nil{return e};p=models.TeacherProfile{SchoolID:j.SchoolID,UserID:u.ID,StaffID:staff,Phone:row["phone"],Gender:row["gender"],Department:row["department"],Designation:row["designation"],Subjects:row["subjects"],Classes:row["classes"],Status:row["status"]};if e=tx.Create(&p).Error;e!=nil{return e};return nil});return created,userEmail,password,role,e}
