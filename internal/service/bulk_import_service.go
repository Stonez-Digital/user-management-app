...
type BulkImportService struct { db *gorm.DB; mu sync.Mutex }
func NewBulkImportService(db *gorm.DB)*BulkImportService{return &BulkImportService{db:db}}
...
func(s *BulkImportService)Create(schoolID,actor uuid.UUID,kind,filename string,rows []BulkRow)(BulkImportPreview,error){
...
func(s *BulkImportService)run(j models.BulkImportJob){s.mu.Lock();defer s.mu.Unlock();var rows []BulkRow;if json.Unmarshal([]byte(j.RowsJSON),&rows)!=nil{s.fail(j.ID,"invalid stored import payload");return};j.Status=models.BulkImportRunning;s.db.Model(&j).Updates(map[string]any{"status":j.Status})
 ...
 for i,row:=range rows{rn:=i+2;if validSet[rn]{continue};if e:=s.processRow(j,row);e!=nil{errs=append(errs,BulkError{rn,"",e.Error()});j.Failed++}else{j.Created++};s.db.Model(&j).Updates(map[string]any{"created":j.Created,"failed":j.Failed,"errors_json":mustJSON(errs)})}
 j.ErrorsJSON=mustJSON(errs);if j.Failed>0||len(errs)>0{j.Status=models.BulkImportCompletedWithErrors}else{j.Status=models.BulkImportCompleted};s.db.Model(&j).Updates(map[string]any{"status":j.Status,"errors_json":j.ErrorsJSON,"failed":j.Failed})
}
...
func(s *BulkImportService)processRow(j models.BulkImportJob,row BulkRow)error{
...
func(s *BulkImportService)createUser(tx *gorm.DB,schoolID uuid.UUID,name,email,role string) (models.User,error){if email==""{email=fmt.Sprintf("%s.%s@import.invalid",strings.ToLower(strings.ReplaceAll(name," ",".")),uuid.NewString()[:8])};var u models.User;if e:=tx.Where("LOWER(email)=?",email).First(&u).Error;e==nil{return u,nil}else if !errors.Is(e,gorm.ErrRecordNotFound){return u,e};hash,e:=auth.HashPassword("ChangeMe-"+uuid.NewString());if e!=nil{return u,e};u=models.User{Name:name,Email:email,PasswordHash:hash,Role:role,Active:true,SchoolID:&schoolID};return u,tx.Create(&u).Error}
func(s *BulkImportService)processStudent(j models.BulkImportJob,name,email string,row BulkRow)error{
 return s.db.Transaction(func(tx *gorm.DB)error{var st models.Student;ad:=strings.TrimSpace(row["admission_number"]);if e:=tx.Where("school_id=? AND admission_number=?",j.SchoolID,ad).First(&st).Error;e==nil{j.Skipped++;return nil}else if !errors.Is(e,gorm.ErrRecordNotFound){return e};u,e:=s.createUser(tx,j.SchoolID,name,email,authz.RoleStudent);if e!=nil{return e};st=models.Student{SchoolID:j.SchoolID,UserID:u.ID,AdmissionNumber:ad,EnrollmentStatus:models.EnrollmentActive,GuardianName:row["guardian_name"],GuardianPhone:row["guardian_phone"],GuardianEmail:row["guardian_email"]};if v:=row["gender"];v!=""{st.Gender=v};if e=tx.Create(&st).Error;e!=nil{return e};return s.maybeEnroll(tx,j.SchoolID,st.ID,row)})
}
func(s *BulkImportService)maybeEnroll(tx *gorm.DB,schoolID,studentID uuid.UUID,row BulkRow)error{sn:=strings.TrimSpace(row["session"]);cn:=strings.TrimSpace(row["class"]);sec:=strings.TrimSpace(row["section"]);if sn==""&&cn==""&&sec==""{return nil};var ses models.AcademicSession;if e:=tx.Where("school_id=? AND name=?",schoolID,sn).First(&ses).Error;e!=nil{return fmt.Errorf("academic session %q not found",sn)};var cl models.SchoolClass;if e:=tx.Where("school_id=? AND name=?",schoolID,cn).First(&cl).Error;e!=nil{return fmt.Errorf("class %q not found",cn)};var se models.Section;if e:=tx.Where("school_id=? AND class_id=? AND name=?",schoolID,cl.ID,sec).First(&se).Error;e!=nil{return fmt.Errorf("section %q not found",sec)};var exists models.StudentEnrollment;if e:=tx.Where("school_id=? AND student_id=? AND academic_session_id=?",schoolID,studentID,ses.ID).First(&exists).Error;e==nil{return nil};return tx.Create(&models.StudentEnrollment{SchoolID:schoolID,StudentID:studentID,AcademicSessionID:ses.ID,ClassID:cl.ID,SectionID:se.ID,Status:models.EnrollmentStatusActive,EnrolledAt:time.Now().UTC()}).Error}
func(s *BulkImportService)processTeacher(j models.BulkImportJob,name,email string,row BulkRow)error{return s.db.Transaction(func(tx *gorm.DB)error{
 staff:=strings.TrimSpace(row["staff_id"]);var p models.TeacherProfile
 if e:=tx.Where("school_id=? AND staff_id=?",j.SchoolID,staff).First(&p).Error;e==nil{return nil}else if !errors.Is(e,gorm.ErrRecordNotFound){return e}
 u,e:=s.createUser(tx,j.SchoolID,name,email,authz.RoleTeacher);if e!=nil{return e}
 p=models.TeacherProfile{SchoolID:j.SchoolID,UserID:u.ID,StaffID:staff,Phone:row["phone"],Gender:row["gender"],Department:row["department"],Designation:row["designation"],Subjects:row["subjects"],Classes:row["classes"],Status:row["status"]}
 if e:=tx.Create(&p).Error;e!=nil{return e};return nil
})}
func(s *BulkImportService)processParent(j models.BulkImportJob,name,email string,row BulkRow)error{return s.db.Transaction(func(tx *gorm.DB)error{
 identifier:=strings.ToLower(strings.TrimSpace(row["parent_identifier"]));var u models.User
 if identifier!=""{var pp models.ParentProfile;if e:=tx.Where("school_id=? AND parent_identifier=?",j.SchoolID,identifier).First(&pp).Error;e==nil{_ = tx.Where("id=?",pp.UserID).First(&u).Error}}
 if u.ID==uuid.Nil&&email!=""{_ = tx.Where("school_id=? AND LOWER(email)=?",j.SchoolID,email).First(&u).Error}
 if u.ID==uuid.Nil{var e error;u,e=s.createUser(tx,j.SchoolID,name,email,authz.RoleParent);if e!=nil{return e};if e=tx.Create(&models.ParentProfile{SchoolID:j.SchoolID,UserID:u.ID,ParentIdentifier:identifier,Phone:row["phone"],Address:row["address"],Occupation:row["occupation"]}).Error;e!=nil{return e}}
 ad:=strings.TrimSpace(row["student_admission_number"]);if ad==""{return nil};var st models.Student;if e:=tx.Where("school_id=? AND admission_number=?",j.SchoolID,ad).First(&st).Error;e!=nil{return fmt.Errorf("student %q not found",ad)};var link models.GuardianRelationship;if e:=tx.Where("school_id=? AND guardian_user_id=? AND student_id=?",j.SchoolID,u.ID,st.ID).First(&link).Error;e==nil{return nil};return tx.Create(&models.GuardianRelationship{SchoolID:j.SchoolID,GuardianUserID:u.ID,StudentID:st.ID,Relationship:strings.TrimSpace(row["relationship"]),Primary:strings.EqualFold(row["primary"],"true")||strings.EqualFold(row["primary"],"yes"),Active:true}).Error
})}
