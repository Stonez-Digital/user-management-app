...
func(s *BulkImportService)processTeacher(j models.BulkImportJob,name,email string,row BulkRow)(bool,bool,string,string,string,error){var created bool;var skipped bool;var password string;userEmail:=email;var role=authz.RoleTeacher;e:=s.db.Transaction(func(tx *gorm.DB)error{staff:=strings.TrimSpace(row["staff_id"]);var p models.TeacherProfile;if e:=tx.Where("school_id=? AND staff_id=?",j.SchoolID,staff).First(&p).Error;e==nil{skipped=true;return nil}else if !errors.Is(e,gorm.ErrRecordNotFound){return e};u,pw,wasCreated,e:=s.createUser(tx,j.SchoolID,name,email,authz.RoleTeacher);password=pw;created=wasCreated;userEmail=u.Email;if e!=nil{return e};p=models.TeacherProfile{SchoolID:j.SchoolID,UserID:u.ID,StaffID:staff,Phone:row["phone"],Gender:row["gender"],Department:row["department"],Designation:row["designation"],Subjects:row["subjects"],Classes:row["classes"],Status:row["status"]};if e=tx.Create(&p).Error;e!=nil{return e};return nil});return created,skipped,userEmail,password,role,e}
func(s *BulkImportService)processParent(j models.BulkImportJob,name,email string,row BulkRow)(bool,bool,string,string,string,error){var created bool;var skipped bool;var password string;userEmail:=email;var role=authz.RoleParent;e:=s.db.Transaction(func(tx *gorm.DB)error{identifier:=strings.ToLower(strings.TrimSpace(row["parent_identifier"]));var u models.User;if identifier!=""{var pp models.ParentProfile;if e:=tx.Where("school_id=? AND parent_identifier=?",j.SchoolID,identifier).First(&pp).Error;e==nil{_ = tx.Where("id=?",pp.UserID).First(&u).Error}};if u.ID==uuid.Nil&&email!=""{if e:=tx.Where("school_id=? AND LOWER(email)=?",j.SchoolID,email).First(&u).Error;e==nil{skipped=true}};if u.ID==uuid.Nil{var e error;u,password,created,e=s.createUser(tx,j.SchoolID,name,email,authz.RoleParent);userEmail=u.Email;if e!=nil{return e};if e=tx.Create(&models.ParentProfile{SchoolID:j.SchoolID,UserID:u.ID,ParentIdentifier:identifier,Phone:row["phone"],Address:row["address"],Occupation:row["occupation"]}).Error;e!=nil{return e}};ad:=strings.TrimSpace(row["student_admission_number"]);if ad==""{return nil};var st models.Student;if e:=tx.Where("school_id=? AND admission_number=?",j.SchoolID,ad).First(&st).Error;e!=nil{return fmt.Errorf("student %q not found",ad)};var link models.GuardianRelationship;if e:=tx.Where("school_id=? AND guardian_user_id=? AND student_id=?",j.SchoolID,u.ID,st.ID).First(&link).Error;e==nil{return nil};return tx.Create(&models.GuardianRelationship{SchoolID:j.SchoolID,GuardianUserID:u.ID,StudentID:st.ID,Relationship:strings.TrimSpace(row["relationship"]),Primary:strings.EqualFold(row["primary"],"true")||strings.EqualFold(row["primary"],"yes"),Active:true}).Error});return created,skipped,userEmail,password,role,e}
