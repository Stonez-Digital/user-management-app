...
func(s *BulkImportService)processTeacher(j models.BulkImportJob,name,email string,row BulkRow)error{_,e:=s.createUser(s.db,j.SchoolID,name,email,authz.RoleTeacher);return e}
func(s *BulkImportService)processParent(j models.BulkImportJob,name,email string,row BulkRow)error{return s.db.Transaction(func(tx *gorm.DB)error{
 ...
 if identifier!=""{_ = tx.Where("school_id=? AND LOWER(email)=?",j.SchoolID,identifier).First(&u).Error};if u.ID==uuid.Nil&&email!=""{_ = tx.Where("school_id=? AND LOWER(email)=?",j.SchoolID,email).First(&u).Error};if u.ID==uuid.Nil{var e error;u,e=s.createUser(tx,j.SchoolID,name,email,authz.RoleParent);if e!=nil{return e}}
