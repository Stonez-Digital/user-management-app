...
func(s *EnrollmentService)Create(schoolID uuid.UUID,v models.StudentEnrollment)(models.StudentEnrollment,error){
 ...
 var session models.AcademicSession;if err:=s.db.Where("id = ? AND school_id = ?",v.AcademicSessionID,schoolID).First(&session).Error;errors.Is(err,gorm.ErrRecordNotFound){return v,ErrEnrollmentSessionMissing}else if err!=nil{return v,err};if session.Status==models.AcademicStatusClosed||session.Status==models.AcademicStatusArchived{return v,ErrEnrollmentSessionUnavailable};if session.Status==models.AcademicStatusClosed||session.Status==models.AcademicStatusArchived{return v,ErrEnrollmentSessionUnavailable}
 ...
 v.Status=strings.ToLower(strings.TrimSpace(v.Status));if v.Status==""{v.Status=models.EnrollmentStatusActive};if v.Status!=models.EnrollmentStatusActive&&v.Status!=models.EnrollmentStatusCompleted&&v.Status!=models.EnrollmentStatusWithdrawn{return v,ErrEnrollmentInvalidStatus};if v.Status==models.EnrollmentStatusActive{var active models.StudentEnrollment;if err:=s.db.Where("school_id = ? AND student_id = ? AND status = ? AND academic_session_id <> ?",schoolID,v.StudentID,models.EnrollmentStatusActive,v.AcademicSessionID).First(&active).Error;err==nil{return v,ErrEnrollmentActiveDuplicate}else if !errors.Is(err,gorm.ErrRecordNotFound){return v,err}};if v.Status==models.EnrollmentStatusActive{var active models.StudentEnrollment;if err:=s.db.Where("school_id = ? AND student_id = ? AND status = ? AND academic_session_id <> ?",schoolID,v.StudentID,models.EnrollmentStatusActive,v.AcademicSessionID).First(&active).Error;err==nil{return v,ErrEnrollmentActiveDuplicate}else if !errors.Is(err,gorm.ErrRecordNotFound){return v,err}}
