...
func(s *TimetableService)validate(v models.TimetableEntry)error{
    ...
    var assignment models.TeacherAssignment;if e:=s.db.First(&assignment,"id=?",v.TeacherAssignmentID).Error;e!=nil||assignment.AcademicSessionID!=v.AcademicSessionID||assignment.TermID!=v.TermID||assignment.ClassID!=v.ClassID{return ErrTimetableInvalid}
    ...
    q:=s.db.Where("academic_session_id=? AND term_id=? AND day_of_week=? AND active=true",v.AcademicSessionID,v.TermID,v.DayOfWeek)
    if v.ID!=uuid.Nil{q=q.Where("id<>?",v.ID)}
    if e:=q.Find(&entries).Error;e!=nil{return e}
    for _,x:=range entries{
        if !overlaps(v.StartTime,v.EndTime,x.StartTime,x.EndTime){continue}
        if x.TeacherAssignmentID==v.TeacherAssignmentID{return ErrTimetableConflict}
        if x.ClassID==v.ClassID&&(x.SectionID==nil||v.SectionID==nil||*x.SectionID==*v.SectionID){return ErrTimetableConflict}
    }
    return nil
}
